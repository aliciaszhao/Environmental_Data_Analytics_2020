...
#take a random assortment of X samples

# Evaluate assumption of normal distribution
shapiro.test((EPAair.subsample$Ozone))

# small p value = NOT normal. null is that distribution is normal. 
...
# using density plot to compare against EPA standard
...
> In conducting the t test, we were interested in finding out whether or not air quality were below the safety threshold (50). The t test returned a a t value of -57.98, a p value of 2.2e-16, which allows us to reject the null hypothesis that air quality is equal to 50, and highly suggests that air quality is less than 50.

> EPA ozone measurements for 2018-2019 were significantly lower than 50, the AQI threshold for good air quality (t-test; t = -57.98, p< .00001)
...
#variance test of Ozone by year
...
# testing that means are different 
...
# For linear model, need to call it by using summar() instead of the object itself
...
Ozone.density <- ggplot(EPAair, aes(x = Ozone)) +
  #geom_density(stat = "count", fill = "gray") +
  geom_density(aes(fill = as.factor(Year)), alpha=.5 )+
  geom_vline(xintercept = 50, color = "black", lty = 2, size = 0.9) +
  scale_x_continuous(expand = c(0, 0)) + scale_y_continuous(expand = c(0, 0)) + 
  labs ( y = "Density", fill ="")
print(Ozone.density)

#2
Ozone.boxplot <- ggplot (EPAair) + 
  geom_boxplot (aes(x = as.factor(Year), y = Ozone)) +
  labs (x = "Year") + 
  geom_text(x = as.factor(2018), y = 45, label = "good", hjust = 1, fontface = "bold") +
  geom_text(x = as.factor(2019), y = 95, label = "moderate", hjust = 1, fontface = "bold") 
