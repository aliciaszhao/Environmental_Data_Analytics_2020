...
**Maps package good for state boundaries and other open source data.
...
# add your code here
Virginia_bounds <- southeast_state_bounds %>%
...
va_basins_nf_seplains <- basins_nf_seplains %>%
  st_intersection(Virginia_bounds)
...
# bonus
# add your code here
va_area <- st_area(Virginia_bounds)
va_centroid <- st_centroid(Virginia_bounds)
va_gages_buffer_100m <- st_buffer(va.gage, dist = 100)
not_va_gages <- st_difference(gages, Virginia_bounds)
```
...
ggplot() +
  geom_sf(data = Virginia_bounds) + # you can add color = "blue" to change outline color to blue or lwd = 2 to change the width of the lines
  theme_bw() # add this to make the background white and not grey
dev.off()
```
...
# add your code here
ggplot() +
      geom_sf(data = Virginia_bounds) +
      geom_sf(data = va_basins_nf_seplains, fill = "pink", alpha =0.25) +
      geom_sf (data = va.gage, color = "black")
dev.off()
```
...
basins_nf_seplains$Overlap <- basins_nf_seplains %>%
  st_intersection(nc_state_bounds_geom) %>%
  logic ()
...
# add your code here
nc_basins_overlap_logic <- basins_nf_seplains %>%
  filter()
...
southeast.gages <- gages %>%
  st_intersection(southeast_state_bounds)

# use nc_basins_overlap_sel to find gages inside
# add your code here
# stretch break...this might take a while to run
nc_basins_overlap_sel <- southeast.gages %>%
  st_intersection(southeast.gages)


# convert all to WGS84
st_transform(southeast.gages, 4326)
nc_basins_nf_seplains.wsg84 <- nc_basins_nf_seplains %>%
  st_transform(4326)
nc.gages.wsg84 <- nc.gages %>%
  st_transform(4326)
...
ggmap(nc_basemap) +
  geom_sf(data = nc_state_bounds_geom_wsg84, fill = NA, lwd = 1, inherit.aes = FALSE) +
  geom_sf(data = nc_basins_nf_seplains.wsg84, fill = "pink", alpha = 0.25, inherit.aes = FALSE ) +
  geom_sf(data = nc.gages.wsg84, color = "black", inherit.aes = FALSE)
dev.off()
