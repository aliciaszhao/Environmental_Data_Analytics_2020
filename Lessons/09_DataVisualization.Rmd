...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  ...
  geom_text(x = as.Date("2020-01-01"), y = 45, label = "good", hjust = 0, fontface = "bold") +
...
NvsP2 <-
  ...
  xlab(expression(paste("Total Phospherous (", mu, "g/L)"))) +
  ylab(expression(paste("Total Nitrogen (", mu, "g/L)"))) +
  labs(color="Depth (m)", shape="Lake Name") +
...
Nutrientplot <-
  ...
  geom_boxplot() +
  labs(y= (expression(paste("Concentration (",mu,"g/L)"))), x="", color="Nutrient")+
