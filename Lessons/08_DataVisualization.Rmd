...
#using print will consistently print in pdf
...
#can manipulate class of Year by using as.factor() within ggplot
...
#facet_wrap means it will wrap around max # of columns/rows
...
#facet grid allows for two variables
...
PM.temp1 <- ggplot(subset(PeterPaul.chem.nutrients, depth==0),
...
PM.temp2 <- ggplot(PeterPaul.chem.nutrients,
                  aes(x = sampledate, y = temperature_C,color=depth), size= 0.5)+
  geom_point() 
...
OzonevPM2.5 <-
  ggplot(EPAair, aes(x = Ozone, y = PM2.5 , color = daily.meanlat), transparency=0.5) +
  geom_point() +
  geom_smooth(method = lm)
print(OzonevPM2.5)  
