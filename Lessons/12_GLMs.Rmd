...
> Depth accoutns for approximately 31% of variance in lake irradiance. 
> Irradiance decreases significantly with decreasing depth (linear regression, R2=0.31, df=15,449, p<0.0001)
> For every 1m increase in dpeht, irradiance decrease by 95 units.  
...
#using scale_y_log10() allows for the values to be unlogged, which are easier to interpret

# SE can also be removed
irradiancebydepth2 <- 
    ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
    geom_point(pch = 1, alpha = 0.5, color  = "dodgerblue") +
    scale_y_log10() +
...
TPplot <- ggplot(subset(PeterPaul.chem.nutrients, lakename == "Paul Lake"), 
                 aes(color = dissolvedOxygen, y = tp_ug, x = depth)) +
  geom_point() +
  xlim(0, 20)
print(TPplot)
...
> Results: A model that includes dissolvedo oxygen, temperature, and total nitrogen explains 26% of TP (linear regression, Adjusted R2 = 2.5,df = 109, p < .0001). TP is significantly affected by dissolved oxygen and temperature.  For every 1 unit increase in dissolved oxygen, TP decreases by 0.83 units. For every 1 unit increase in temperature, TP decreases by .48 units. 