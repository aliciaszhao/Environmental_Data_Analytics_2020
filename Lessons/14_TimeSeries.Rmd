...
# Use drop_na when we want to exclude from one column. 
...
EnoDischarge.late_ts2 <- ts(EnoDischarge.late$discharge.mean, frequency = 365)
```
...
EnoDischarge.late_Decomposed <- stl(EnoDischarge.late_ts2, s.window = "periodic")

# Visualize the decomposed series. 
plot(EnoDischarge.late_Decomposed)

# Scales of seasonal and trend are so small that most of remainder ends up looking like original data. 
...
> The discharge mean for Eno River from 1927-1971 are significantly higher from the discharge mean from 1985-2019 (Wilcoxon, W=1.17e^8, p< .0001). We could use a density plot to represent them graphically. 
...
> aggregation: taking multiple data points to create one data point. Good for getting around data gaps. 

> subsampling: Taking one point for each month to represent data.

> interpolation: fill in gaps between points 

Common interpolation methods: 

* **Piecewise constant**: also known as a "nearest neighbor" approach. Any missing data are assumed to be equal to the measurement made nearest to that date (could be earlier or later). Looks like a step function. 
...
# general output of test. Null hyp is that therei s no monotonic trend over time. High p value means that there is no trend over time.  Slightly decreasing trend but not significant. 
...
> Discharge from 1985-2019 does not appear to display a significant trend across any individual months beyond the seasonal trends (seasonal Mann-Kendall, z= -0.14, p = 0.889). 
