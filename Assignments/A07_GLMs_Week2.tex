...
\begin{Highlighting}[]
\CommentTok{#3}
\NormalTok{NTL.July <-}\StringTok{ }\NormalTok{NTL.chem }\OperatorTok{%>%}
...
\CommentTok{#4}
\NormalTok{Temp_AIC <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ NTL.July, temperature_C }\OperatorTok{~}\StringTok{ }\NormalTok{year4 }\OperatorTok{+}\StringTok{ }\NormalTok{daynum }\OperatorTok{+}\StringTok{ }
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#The AIC test recommends that we keep all variables. }

\NormalTok{Tempmodel <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ NTL.July, temperature_C}\OperatorTok{~}\NormalTok{year4 }\OperatorTok{+}\StringTok{ }\NormalTok{daynum }\OperatorTok{+}\StringTok{ }\NormalTok{depth)}
...
\begin{quote}
Answer: The final set of explanatory variables are year, day number, and
depth, as this model yielded the lowest AIC. This model explains
approximately 74\% of the observed variation in temperature.
\end{quote}
...
\begin{Highlighting}[]
\CommentTok{#6}
\NormalTok{Tempancova.interaction <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ NTL.July, temperature_C }\OperatorTok{~}\StringTok{ }\NormalTok{lakename }\OperatorTok{*}\StringTok{ }\NormalTok{depth)}
...
for all lakes except for Paul Lake. The model with an interaction effect
explains approximately 79\% of the variation.
\end{quote}
...
\NormalTok{NTL.tempmodel <-}
\KeywordTok{ggplot}\NormalTok{(NTL.July, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ depth, }\DataTypeTok{y =}\NormalTok{ temperature_C, }\DataTypeTok{color =}\NormalTok{ lakename)) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_point}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{alpha=}\FloatTok{0.5}\NormalTok{)) }\OperatorTok{+}
\StringTok{  }\KeywordTok{scale_color_manual}\NormalTok{(}\DataTypeTok{values=}\KeywordTok{c}\NormalTok{(}\StringTok{"blue"}\NormalTok{, }\StringTok{"red"}\NormalTok{, }\StringTok{"orange"}\NormalTok{, }\StringTok{"darkgreen"}\NormalTok{, }\StringTok{"purple"}\NormalTok{, }\StringTok{"hotpink"}\NormalTok{, }\StringTok{"black"}\NormalTok{, }\StringTok{"gray"}\NormalTok{, }\StringTok{"brown"}\NormalTok{)) }\OperatorTok{+}
...
\KeywordTok{print}\NormalTok{(NTL.tempmodel)}
\end{Highlighting}
\end{Shaded}
...
\StringTok{  }\NormalTok{Litter }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{na.exclude}\NormalTok{()}

\NormalTok{Drymass.mixed <-}\StringTok{ }\KeywordTok{lme}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ Litter.processed,}
...
\KeywordTok{summary}\NormalTok{(Drymass.mixed)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{rsquared}\NormalTok{(Drymass.mixed)}
\end{Highlighting}
\end{Shaded}
...
\begin{quote}
Answer: Approximately 2\% additional variance is explained by adding the
random effect to the model.
\end{quote}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Drymass.fixed <-}\StringTok{ }\KeywordTok{gls}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ Litter.processed,}
\NormalTok{                    dryMass }\OperatorTok{~}\StringTok{ }\NormalTok{nlcdClass}\OperatorTok{*}\NormalTok{functionalGroup)}
\KeywordTok{summary}\NormalTok{(Drymass.fixed)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{anova}\NormalTok{(Drymass.mixed, Drymass.fixed)}
\end{Highlighting}
\end{Shaded}
...
\begin{quote}
Answer: The mixed effect seems to be a better model than the fixed
effects model because it has a lower AIC (\textasciitilde{}9038 versus
\textasciitilde{}9058).
\end{quote}
