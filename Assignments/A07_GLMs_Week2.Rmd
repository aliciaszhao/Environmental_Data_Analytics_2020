...
#4
Temp_AIC <- lm(data = NTL.July, temperature_C ~ year4 + daynum + 
              depth)
step(Temp_AIC)

#The AIC test recommends that we keep all variables. 

Tempmodel <- lm(data = NTL.July, temperature_C~year4 + daynum + depth)
summary(Tempmodel)
...
> Answer: The final set of explanatory variables are year, day number, and depth, as this model yielded the lowest AIC. This model explains approximately 74% of the observed variation in temperature. 

6. Run an interaction effects ANCOVA to predict temperature based on depth and lakename from the same wrangled dataset.

```{r}
#6
Tempancova.interaction <- lm(data = NTL.July, temperature_C ~ lakename * depth)
...
> Answer: There is a significant interaction between depth and lakename for all lakes except for Paul Lake. The model with an interaction effect explains approximately 79% of the variation. 
...
ggplot(NTL.July, aes(x = depth, y = temperature_C, color = lakename)) +
  ...
  scale_color_manual(values=c("blue", "red", "orange", "darkgreen", "purple", "hotpink", "black", "gray", "brown")) +
...
Drymass.mixed <- lme(data = Litter.processed,
...
> Answer: Approximately 2% additional variance is explained by adding the random effect to the model. 

c. Run the same model without the random effect.
d. Run an anova on the two tests. 
```{r}
 
Drymass.fixed <- gls(data = Litter.processed,
                    dryMass ~ nlcdClass*functionalGroup)
summary(Drymass.fixed)
...
> Answer: The mixed effect seems to be a better model than the fixed effects model because it has a lower AIC (~9038 versus ~9058). 