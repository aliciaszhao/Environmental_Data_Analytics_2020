...
\NormalTok{EPAair.O3}\FloatTok{.2018}\NormalTok{ <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"../Data/Raw/EPAair_O3_NC2018_raw.csv"}\NormalTok{)}
\NormalTok{EPAair.O3}\FloatTok{.2019}\NormalTok{ <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"../Data/Raw/EPAair_O3_NC2019_raw.csv"}\NormalTok{)}
\NormalTok{EPAair.PM25}\FloatTok{.2018}\NormalTok{ <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"../Data/Raw/EPAair_PM25_NC2018_raw.csv"}\NormalTok{)}
\NormalTok{EPAair.PM25}\FloatTok{.2019}\NormalTok{ <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"../Data/Raw/EPAair_PM25_NC2019_raw.csv"}\NormalTok{)}

\CommentTok{#2}
\CommentTok{#looking at dimension of datasets}
\KeywordTok{dim}\NormalTok{(EPAair.O3}\FloatTok{.2018}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{dim}\NormalTok{(EPAair.O3}\FloatTok{.2019}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{dim}\NormalTok{(EPAair.PM25}\FloatTok{.2018}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\CommentTok{#3}
\CommentTok{#Change 'date' to date}
\NormalTok{EPAair.O3}\FloatTok{.2018}\OperatorTok{$}\NormalTok{Date <-}\StringTok{ }\KeywordTok{as.Date}\NormalTok{(EPAair.O3}\FloatTok{.2018}\OperatorTok{$}\NormalTok{Date, }\DataTypeTok{format =} \StringTok{"%m/%d/%Y"}\NormalTok{)}
\NormalTok{EPAair.O3}\FloatTok{.2019}\OperatorTok{$}\NormalTok{Date <-}\StringTok{ }\KeywordTok{as.Date}\NormalTok{(EPAair.O3}\FloatTok{.2019}\OperatorTok{$}\NormalTok{Date, }\DataTypeTok{format =} \StringTok{"%m/%d/%Y"}\NormalTok{)}
\NormalTok{EPAair.PM25}\FloatTok{.2018}\OperatorTok{$}\NormalTok{Date <-}\StringTok{ }\KeywordTok{as.Date}\NormalTok{(EPAair.PM25}\FloatTok{.2018}\OperatorTok{$}\NormalTok{Date, }\DataTypeTok{format =} \StringTok{"%m/%d/%Y"}\NormalTok{)}
\NormalTok{EPAair.PM25}\FloatTok{.2019}\OperatorTok{$}\NormalTok{Date <-}\StringTok{ }\KeywordTok{as.Date}\NormalTok{(EPAair.PM25}\FloatTok{.2019}\OperatorTok{$}\NormalTok{Date, }\DataTypeTok{format =} \StringTok{"%m/%d/%Y"}\NormalTok{)}

\CommentTok{#4}
\NormalTok{EPAair.O3.}\FloatTok{2018.}\NormalTok{subset <-}\StringTok{ }\KeywordTok{select}\NormalTok{(EPAair.O3}\FloatTok{.2018}\NormalTok{,}
\NormalTok{                                Date, DAILY_AQI_VALUE, Site.Name,}
\NormalTok{                                AQS_PARAMETER_DESC, COUNTY, SITE_LATITUDE,}
\NormalTok{                                SITE_LONGITUDE)}

\NormalTok{EPAair.O3.}\FloatTok{2019.}\NormalTok{subset <-}\StringTok{ }\KeywordTok{select}\NormalTok{(EPAair.O3}\FloatTok{.2019}\NormalTok{, }
\NormalTok{                                Date, DAILY_AQI_VALUE, Site.Name,}
\NormalTok{                                AQS_PARAMETER_DESC, COUNTY, SITE_LATITUDE,}
\NormalTok{                                SITE_LONGITUDE)}

\NormalTok{EPAair.PM25.}\FloatTok{2018.}\NormalTok{subset <-}\StringTok{ }\KeywordTok{select}\NormalTok{ (EPAair.PM25}\FloatTok{.2018}\NormalTok{,}
\NormalTok{                                   Date, DAILY_AQI_VALUE, Site.Name,}
\NormalTok{                                   AQS_PARAMETER_DESC, COUNTY, SITE_LATITUDE,}
\NormalTok{                                   SITE_LONGITUDE)}

\NormalTok{EPAair.PM25.}\FloatTok{2019.}\NormalTok{subset <-}\StringTok{ }\KeywordTok{select}\NormalTok{ (EPAair.PM25}\FloatTok{.2019}\NormalTok{,}
\NormalTok{                                   Date, DAILY_AQI_VALUE, Site.Name,}
\NormalTok{                                   AQS_PARAMETER_DESC, COUNTY, SITE_LATITUDE,}
\NormalTok{                                   SITE_LONGITUDE)}
...
\NormalTok{EPAair.PM25.}\FloatTok{2018.}\NormalTok{subset}\OperatorTok{$}\NormalTok{AQS_PARAMETER_DESC <-}\StringTok{ "PM2.5"}
\NormalTok{EPAair.PM25.}\FloatTok{2019.}\NormalTok{subset}\OperatorTok{$}\NormalTok{AQS_PARAMETER_DESC <-}\StringTok{ "PM2.5"}

\CommentTok{#6}

\KeywordTok{write.csv}\NormalTok{(EPAair.O3.}\FloatTok{2018.}\NormalTok{subset, }\DataTypeTok{row.names =} \OtherTok{FALSE}\NormalTok{, }
          \DataTypeTok{file =}\StringTok{"../Data/Processed/EPAair_O3_NC2018_Processed.csv"}\NormalTok{)}

\KeywordTok{write.csv}\NormalTok{(EPAair.O3.}\FloatTok{2019.}\NormalTok{subset, }\DataTypeTok{row.names =} \OtherTok{FALSE}\NormalTok{, }
          \DataTypeTok{file =}\StringTok{"../Data/Processed/EPAair_O3_NC2019_Processed.csv"}\NormalTok{)}

\KeywordTok{write.csv}\NormalTok{(EPAair.PM25.}\FloatTok{2018.}\NormalTok{subset, }\DataTypeTok{row.names =} \OtherTok{FALSE}\NormalTok{, }
          \DataTypeTok{file =}\StringTok{"../Data/Processed/EPAair_PM25_NC2018_Processed.csv"}\NormalTok{)}

\KeywordTok{write.csv}\NormalTok{(EPAair.PM25.}\FloatTok{2019.}\NormalTok{subset, }\DataTypeTok{row.names =} \OtherTok{FALSE}\NormalTok{, }
...
\begin{Highlighting}[]
\CommentTok{#7}
\NormalTok{EPAair.O3.PM25 <-}\StringTok{ }\KeywordTok{rbind}\NormalTok{(EPAair.O3.}\FloatTok{2018.}\NormalTok{subset,EPAair.O3.}\FloatTok{2019.}\NormalTok{subset, EPAair.PM25.}\FloatTok{2018.}\NormalTok{subset, EPAair.PM25.}\FloatTok{2019.}\NormalTok{subset)}

\CommentTok{#8}
\NormalTok{EPAair.O3.PM25.filter <-}\StringTok{ }\NormalTok{EPAair.O3.PM25 }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{filter}\NormalTok{ (Site.Name }\OperatorTok{==}\StringTok{ "Linville Falls"}\OperatorTok{|}
\StringTok{            }\NormalTok{Site.Name}\OperatorTok{==}\StringTok{"Durham Armory"}\OperatorTok{|}
\StringTok{            }\NormalTok{Site.Name }\OperatorTok{==}\StringTok{"Leggett"}\OperatorTok{|}
\StringTok{            }\NormalTok{Site.Name }\OperatorTok{==}\StringTok{"Hattie Avenue"}\OperatorTok{|}
\StringTok{            }\NormalTok{Site.Name }\OperatorTok{==}\StringTok{ "Clemmons Middle"}\OperatorTok{|}
\StringTok{            }\NormalTok{Site.Name }\OperatorTok{==}\StringTok{ "Mendenhall School"}\OperatorTok{|}
\StringTok{            }\NormalTok{Site.Name }\OperatorTok{==}\StringTok{ "Frying Pan Mountain"}\OperatorTok{|}
\StringTok{            }\NormalTok{Site.Name }\OperatorTok{==}\StringTok{ "West Johnston Co."}\OperatorTok{|}
\StringTok{            }\NormalTok{Site.Name }\OperatorTok{==}\StringTok{ "Garinger High School"}\OperatorTok{|}
\StringTok{            }\NormalTok{Site.Name }\OperatorTok{==}\StringTok{ "Castle Hayne"}\OperatorTok{|}
\StringTok{            }\NormalTok{Site.Name }\OperatorTok{==}\StringTok{ "Pitt Agri. Center"}\OperatorTok{|}
\StringTok{            }\NormalTok{Site.Name }\OperatorTok{==}\StringTok{ "Bryson City"}\OperatorTok{|}
...
\StringTok{  }\KeywordTok{summarise}\NormalTok{(}\DataTypeTok{daily.meanaqi =} \KeywordTok{mean}\NormalTok{(DAILY_AQI_VALUE), }
            \DataTypeTok{daily.meanlat =} \KeywordTok{mean}\NormalTok{(SITE_LATITUDE), }
            \DataTypeTok{daily.meanlong =} \KeywordTok{mean}\NormalTok{(SITE_LONGITUDE)) }\OperatorTok{%>%}
...
\KeywordTok{dim}\NormalTok{ (EPAair.O3.PM25.filter)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\CommentTok{#9}
\NormalTok{EPAair.O3.PM25.filter.spread <-}\StringTok{ }\KeywordTok{spread}\NormalTok{(EPAair.O3.PM25.filter, AQS_PARAMETER_DESC, daily.meanaqi)}

\CommentTok{#10}
\KeywordTok{dim}\NormalTok{(EPAair.O3.PM25.filter.spread)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\CommentTok{#11}
\KeywordTok{write.csv}\NormalTok{(EPAair.O3.PM25.filter.spread, }\DataTypeTok{row.names =} \OtherTok{FALSE}\NormalTok{, }
          \DataTypeTok{file =}\StringTok{"../Data/Processed/EPAair_O3_PM25_NC1819_Processed.csv"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\CommentTok{#12a}
\NormalTok{EPAair.O3.PM25.summary <-}\StringTok{ }\NormalTok{EPAair.O3.PM25.filter.spread }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{group_by}\NormalTok{(Site.Name, Month, Year) }\OperatorTok{%>%}
\StringTok{   }\KeywordTok{summarise}\NormalTok{(}\DataTypeTok{Mean.Ozone=} \KeywordTok{mean}\NormalTok{(Ozone), }
             \DataTypeTok{Mean.PM2.5=} \KeywordTok{mean}\NormalTok{(PM2}\FloatTok{.5}\NormalTok{))}

\CommentTok{#12b}

\NormalTok{EPAair.O3.PM25.summary.clean <-}\StringTok{ }\NormalTok{EPAair.O3.PM25.summary }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{drop_na}\NormalTok{(Month, Year)}

\CommentTok{#13}

\KeywordTok{dim}\NormalTok{ (EPAair.O3.PM25.summary.clean)}
\end{Highlighting}
\end{Shaded}
...
\begin{quote}
Answer: drop\_na allows us to remove entire rows that contain missing
values, whereas na.omit only removes the missing values and not the row
that they're in.
\end{quote}
