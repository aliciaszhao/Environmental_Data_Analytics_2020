...
\NormalTok{GaringerOzone.filtered <-}\KeywordTok{select}\NormalTok{(GaringerOzone, Date, Daily.Max.}\FloatTok{8.}\NormalTok{hour.Ozone.Concentration,}
...
\CommentTok{# 6}
\NormalTok{GaringerOzone.Date <-}\StringTok{ }\KeywordTok{left_join}\NormalTok{(Days, GaringerOzone.filtered)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{GaringerOzone.plot <-}\StringTok{ }\KeywordTok{ggplot}\NormalTok{(GaringerOzone.Date, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ Date, }\DataTypeTok{y =}\NormalTok{ Daily.Max.}\FloatTok{8.}\NormalTok{hour.Ozone.Concentration)) }\OperatorTok{+}\StringTok{ }
\StringTok{  }\KeywordTok{geom_line}\NormalTok{() }\OperatorTok{+}\StringTok{ }
\StringTok{  }\KeywordTok{labs}\NormalTok{ ( }\DataTypeTok{x =} \StringTok{""}\NormalTok{, }\DataTypeTok{y =} \StringTok{"Ozone Concentration (ppm)"}\NormalTok{) }\OperatorTok{+}\StringTok{ }
...
\begin{quote}
Answer: We do not use the piecewise constant, because values seem to be
gradually changing over time. We do not use the spline interpolation
because the data do not appear to have a cubic relationship, and there
is the potential issue of overshooting.
\end{quote}
...
\begin{quote}
Answer: Ozone concentration seems to display a significant seasonal
trend, according to the line graph of the data, as well as the
decomposed time series. This makes the SMK test most appropriate, as it
...
\begin{Highlighting}[]
\CommentTok{# 8}
\NormalTok{GaringerOzone.Date}\OperatorTok{$}\NormalTok{Ozone <-}\StringTok{ }\KeywordTok{na.approx}\NormalTok{(GaringerOzone.Date}\OperatorTok{$}\NormalTok{Daily.Max.}\FloatTok{8.}\NormalTok{hour.Ozone.Concentration)}

\CommentTok{# 9}
\NormalTok{GaringerOzone.monthly <-}\StringTok{ }\NormalTok{GaringerOzone.Date }\OperatorTok{%>%}
...
\StringTok{  }\KeywordTok{drop_na}\NormalTok{(Daily.Max.}\FloatTok{8.}\NormalTok{hour.Ozone.Concentration) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{group_by}\NormalTok{ (Year, Month) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{summarise}\NormalTok{ (}\DataTypeTok{Ozone.Concentration =} \KeywordTok{mean}\NormalTok{(Daily.Max.}\FloatTok{8.}\NormalTok{hour.Ozone.Concentration))}
...
\CommentTok{# 10}
\NormalTok{GaringerOzone.monthly.ts <-}\StringTok{ }\KeywordTok{ts}\NormalTok{(GaringerOzone.monthly}\OperatorTok{$}\NormalTok{Ozone.Concentration, }
                               \DataTypeTok{start =} \KeywordTok{c}\NormalTok{(}\DecValTok{2010}\NormalTok{, }\DecValTok{1}\NormalTok{, }\DecValTok{1}\NormalTok{),}
                               \DataTypeTok{end =} \KeywordTok{c}\NormalTok{(}\DecValTok{2019}\NormalTok{, }\DecValTok{12}\NormalTok{, }\DecValTok{1}\NormalTok{), }\DataTypeTok{frequency =}\DecValTok{12}\NormalTok{)}

\CommentTok{# 11}
\NormalTok{GaringerOzone_Decomposed <-}\StringTok{ }\KeywordTok{stl}\NormalTok{(GaringerOzone.monthly.ts, }\DataTypeTok{s.window =} \StringTok{"periodic"}\NormalTok{)}
\KeywordTok{plot}\NormalTok{(GaringerOzone_Decomposed)}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{ggplot}\NormalTok{(GaringerOzone.monthly, }\KeywordTok{aes}\NormalTok{ (}\DataTypeTok{x =}\NormalTok{ Date, }\DataTypeTok{y =}\NormalTok{ Ozone.Concentration)) }\OperatorTok{+}\StringTok{ }
...
\begin{quote}
Answer: Ozone concentrations from 2010-2019 display a slightly negative
trend across individual months (Seasonal Mann-Kendall, z= -2.25, p =
0.025).
\end{quote}
